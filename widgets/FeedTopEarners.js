...
export default function FeedTopEarners({ ...props }) {
  const data = [
    {
      hours: 271,
    ...
    <tr key={index}>
      ...
      <td>
        <Row className="align-items-center g-0">
          ...
          <Col>
            <ProgressBar now={item.progress} variant="secondary" className="progress-sm" />
          </Col>
        </Row>
      </td>
      <td>{item.rate}%</td>
  ...
  return (
    <Card {...props}>
      <Card.Header>
        <h4 className="card-header-title">Top Earners</h4>
      </Card.Header>
